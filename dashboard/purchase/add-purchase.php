<?php
$title = "Dashboard - Add Purchasing";
...
function add_purchase($data)
{
    ...
    $stmt = $conn->prepare("INSERT INTO purchases (supplier_id, product_id, purchase_date, quantity, total) VALUES (?, ?, ?, ?, ?)");
    if ($stmt === false) {
        return "Query gagal: " . $conn->error;
    }

    $stmt->bind_param("iissi", $data['supplier_id'], $data['product_id'], $data['purchase_date'], $data['quantity'], $data['total']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = [
        ...
        'purchase_date' => date('Y-m-d H:i:s', strtotime($_POST['purchase_date'])), // Format tanggal untuk MySQL
    ...
    if (empty($errors)) {
        $result = add_purchase($data);
        if ($result === true) {
            $message = '
            <div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50" role="alert">
                <span class="font-medium">Berhasil menambahkan data pembelian!</span>
            </div>';
...
<h1 class="text-base lg:text-xl">Tambah Pembelian</h1>
        ...
        <div>
            ...
            <select required id="product_id" name="product_id" class="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" onchange="updateCurrentTotal()">
                ...
                <?php foreach ($products as $index => $product) : ?>
                    <option price="<?php echo htmlspecialchars($product['price']); ?>" value="<?php echo htmlspecialchars($product['id']); ?>"><?php echo htmlspecialchars($product['name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div>
            <label for="quantity" class="block mb-1 text-sm font-medium text-gray-900">Jumlah Unit</label>
            <input required type="number" name="quantity" id="quantity" value="<?php echo isset($_POST['quantity']) ? htmlspecialchars($_POST['quantity']) : ''; ?>" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500" oninput="updateCurrentTotal()">
        </div>
        <div>
            <label for="total" class="block mb-1 text-sm font-medium text-gray-900">Total Harga</label>
            <input required readonly type="number" name="total" id="update-total" value="<?php echo isset($_POST['total']) ? htmlspecialchars($_POST['total']) : ''; ?>" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500">
        </div>
        <div>
            <label for="purchase_date" class="block mb-1 text-sm font-medium text-gray-900 ">Tanggal Pembelian</label>
            <input required type="datetime-local" name="purchase_date" id="purchase_date" value="<?php echo isset($_POST['purchase_date']) ? htmlspecialchars($_POST['purchase_date']) : ''; ?>" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500">
        </div>
